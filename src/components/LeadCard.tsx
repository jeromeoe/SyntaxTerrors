...
export function LeadCard({ lead }: LeadCardProps) {
  ...
  const ScoreItem = ({ 
    ...
    <div className="flex items-center gap-2" role="group" aria-label={`${label}: ${score}`}>
      ...
      <span className="text-gray-700">{label} ({weight * 100}%):</span>
  ...
  return (
    <article className="bg-white rounded-lg shadow-lg p-6 hover:shadow-xl transition-shadow">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        ...
        <ScoreItem score={lead.practicality} label="Practicality" icon={PenTool} weight={SCORE_WEIGHTS.practicality} />
        <ScoreItem score={lead.revenue} label="Revenue" icon={DollarSign} weight={SCORE_WEIGHTS.revenue} />
        <ScoreItem score={lead.aiEase} label="AI Ease" icon={CheckCircle} weight={SCORE_WEIGHTS.aiEase} />
        <ScoreItem score={lead.difficulty} label="Difficulty" icon={Brain} weight={SCORE_WEIGHTS.difficulty} />
      </div>
            ...
            {lead.scoringDetails?.penalties.map((penalty, idx) => (
              <li key={idx} className="text-sm">
                ...
                <div className="text-xs text-gray-700">
                  Applied penalty: {(penalty.penaltyFactor * 100).toFixed(1)}% ({penalty.penaltyValue.toFixed(1)} points)
                </div>
              </li>
            ))}
          </ul>
